...
> **优先级说明**：这两项应排在任何架构演进之前。在一个存在任意文件删除漏洞的服务上讨论「下一个形态」是本末倒置。同时建议将 `mvp-evaluation.md` 恢复进 `docs/`——自评报告属于长期资产，不应随知识库迁移被删除。
